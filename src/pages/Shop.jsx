  ...
  return <div className="shop-page">
    ...
    <section className="section-wrap shop-layout">
      <aside className="filters">
        <div className="filter-title"><SlidersHorizontal/><strong>Categories</strong></div>
        <div className="filter-group"><span>Browse</span>{categories.map(c=><button className={category===c?'active':''} key={c} onClick={()=>chooseCategory(c)}>{c}<em>{c==='All'?products.length:products.filter(p=>p.category===c).length}</em></button>)}</div>
      ...
      <div className="shop-results">
        <div className="shop-toolbar">
          <div className="search-field"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search phones, laptops, gaming, accessories..."/>{query&&<button onClick={()=>setQuery('')}><X/></button>}</div>
          <div className="result-count">{filtered.length} products</div>
          <select value={sort} onChange={e=>setSort(e.target.value)} aria-label="Sort products"><option value="featured">Featured</option><option value="low">Price: low to high</option><option value="high">Price: high to low</option><option value="sale">Sale items first</option></select>
        </div>
